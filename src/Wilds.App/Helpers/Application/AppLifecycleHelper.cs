...
using Wilds.App.Helpers.Application;
using Wilds.App.Services.SizeProvider;
using Wilds.App.Utils.Logger;
...
namespace Wilds.App.Helpers
{
	...
	public static class AppLifecycleHelper
	{
		...
		/// </summary>
		public static Version AppVersion { get; } = WildsAppInfo.Version;

		/// <summary>
		/// Gets application icon path.
		/// </summary>
		public static string AppIconPath { get; } =
			SystemIO.Path.Combine(WildsAppInfo.InstalledPath, AppEnvironment switch
			{
		...
		public static void ConfigureSentry()
		{
			SentrySdk.Init(options =>
			{
				...
				var appVersion = WildsAppInfo.Version;
				...
				options.Environment = AppEnvironment == AppEnvironment.Dev ? "dev" : "production";
		...
		public static IHost ConfigureHost()
		{
			var builder = Host.CreateDefaultBuilder()
				.UseContentRoot(WildsAppInfo.InstalledPath)
			...
			// Velopack 経由で GitHub Releases から更新する単一実装。
			// Dev ビルドでは VelopackUpdateService 内で IsInstalled=false となり自動的に no-op になる。
			builder.ConfigureServices(s => s.AddSingleton<IUpdateService, VelopackUpdateService>());
