	...
	public static partial class Win32Helper
	{
			...
			IconOptions iconOptions)
		{
			...
			try
			{
				...
				if (shellItem is not null && shellItem.IShellItem is Shell32.IShellItemImageFactory shellFactory)
				{
					...
					{
						// Why (P0 #2): Image.FromHbitmap は HBITMAP を所有しない。
						// Bitmap.Dispose (using スコープ終了) 後に HBITMAP を GDI から明示削除しないと
						// 起動中アイコン取得のたびに GDI ハンドルがリークし続ける。
						if (!hbitmap.IsNull)
							Gdi32.DeleteObject(hbitmap);
					}

					// Why (P0 #1): shellFactory は shellItem.IShellItem を QueryInterface でキャストしたもの。
					// 同一 COM オブジェクトを指す RCW なので、ここで ReleaseComObject すると後続の
					// shellItem.Dispose() (using スコープ終了時) で InvalidComObjectException が発生する
					// (起動直後から連続発生していた警告の真因)。using に任せて二重解放を避ける。
				}
		...
		public static bool SetCustomDirectoryIcon(string? folderPath, string? iconFile, int iconIndex = 0)
		{
			...
			var fcs = new Shell32.SHFOLDERCUSTOMSETTINGS()
			{
				...
				// Vanara 5.x で pszIconFile の型が StrPtrAuto に変更された。
				pszIconFile = iconFile is null ? default : new Vanara.InteropServices.StrPtrAuto(iconFile),
				cchIconFile = 0,
