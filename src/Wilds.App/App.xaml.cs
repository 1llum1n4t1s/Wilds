	...
	public partial class App : Application
	{
		...
		protected override void OnLaunched(LaunchActivatedEventArgs e)
		{
			...
			async Task ActivateAsync()
			{
				...
				// Unpackaged 配布では MSIX の IStartupTaskActivatedEventArgs は来ない (Run レジストリ起動のみ)
				var isStartupTask = false;
		...
		private async void Window_Closed(object sender, WindowEventArgs args)
		{
			...
			if (userSettingsService.GeneralSettingsService.LeaveAppRunning &&
				...
				!Process.GetProcessesByName("Wilds").Any(x => x.Id != Environment.ProcessId))
			{
			...
			SafetyExtensions.IgnoreExceptions(() =>
			{
				...
				if (dataPackage.Properties.PackageFamilyName == WildsAppInfo.FamilyName)
				{
