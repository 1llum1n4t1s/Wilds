...
namespace Wilds.App.Utils.Cloud
{
	...
	public sealed class SynologyDriveCloudDetector : AbstractCloudDetector
	{
		protected override async IAsyncEnumerable<ICloudProvider> GetProviders()
		{
			...
			var tempFolder = await StorageFolder.GetFolderFromPathAsync(AppPaths.TemporaryFolderPath);
			await configFile.CopyAsync(tempFolder, "synology_drive.db", NameCollisionOption.ReplaceExisting);

			var syncDbPath = Path.Combine(AppPaths.TemporaryFolderPath, "synology_drive.db");
