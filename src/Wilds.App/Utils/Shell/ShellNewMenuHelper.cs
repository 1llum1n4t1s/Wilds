...
namespace Wilds.App.Utils.Shell
{
	/// <summary>
	...
	public static class ShellNewMenuHelper
	{
		...
		{
			// Unpackaged 配布では LocalFolder が StorageFolder として取得できないため、
			// 実パスから取得する。
			var extensionsPath = SystemIO.Path.Combine(AppPaths.LocalFolderPath, "extensions");
			SystemIO.Directory.CreateDirectory(extensionsPath);
			var folder = await SafetyExtensions.IgnoreExceptions(() => StorageFolder.GetFolderFromPathAsync(extensionsPath).AsTask());
