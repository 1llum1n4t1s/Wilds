...
namespace Wilds.App.Data.Items
{
	...
	public unsafe partial class WindowEx : Window, IDisposable
	{
		...
		// Unpackaged 配布では ApplicationData.Current.LocalSettings が使えないため、
		// AppSettingsStore を裏に持つ簡易な IDictionary&lt;string, object?&gt; で代替する。
		private readonly IDictionary<string, object?> _applicationDataContainer = AppSettingsStore.Values;
		...
		{
			// ApplicationDataContainer 時代の "Files" / "WinUIEx" のサブコンテナは
			// 全て AppSettingsStore 直下のフラットキーに集約する。旧データ互換は捨てる。
			oldDataExists = false;
			return _applicationDataContainer;
		}
