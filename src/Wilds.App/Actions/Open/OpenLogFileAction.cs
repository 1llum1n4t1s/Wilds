...
namespace Wilds.App.Actions
{
	...
	internal sealed partial class OpenLogFileAction : IAction
	{
		...
		public async Task ExecuteAsync(object? parameter = null)
		{
			try
			{
				var debugFilePath = SystemIO.Path.Combine(AppPaths.LocalFolderPath, "debug.log");
				if (!SystemIO.File.Exists(debugFilePath))
					return;

				using var process = Process.Start(new ProcessStartInfo(debugFilePath)
				{
